...
from common import AV_STATUS_FULL
from common import AV_BUCKETS_TO_SCAN
from common import AV_SCANNER_LAMBDA

# Get all objects in an S3 bucket that have not been complete-s3ly scanned
...
def lambda_handler(event, context):
    ...
    # We Will Pass this with Environment Variables, Instead of Arguments
    # parser.add_argument(
    ...
    print(AV_BUCKETS_TO_SCAN)
    print("Lambda Function Name :", AV_SCANNER_LAMBDA)
    print("Targetted S3 Buckets :", AV_BUCKETS_TO_SCAN)
    if "," in AV_BUCKETS_TO_SCAN:
        for i in AV_BUCKETS_TO_SCAN.split(","):
            print(i)
            main(AV_SCANNER_LAMBDA, i, args.limit)
        return True
    else:
        main(AV_SCANNER_LAMBDA, AV_BUCKETS_TO_SCAN, args.limit)
        return True