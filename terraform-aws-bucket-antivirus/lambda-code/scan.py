...
from common import AV_MOVE_INFECTED_FILES
...
from common import DESTINATION_BUCKET_NAME
...
def copy_s3_object(s3_object):
    copy_source_object = {'Bucket': s3_object.bucket_name, 'Key': s3_object.key}
    print(s3_object.bucket_name)
    print(s3_object.key)
    print(DESTINATION_BUCKET_NAME)
    try:
        s3_client.copy_object(CopySource=copy_source_object, Bucket=DESTINATION_BUCKET_NAME, Key=s3_object.key)
    except Exception:
        raise Exception(
            "Failed to copy infected file: %s.%s"
            % (s3_object.bucket_name, s3_object.key)
        )
    else:
        print("Infected file copied: %s.%s" % (s3_object.bucket_name, s3_object.key))
...
def lambda_handler(event, context):
    ...
    try:
        ...
        pass
    if str_to_bool(AV_MOVE_INFECTED_FILES) and scan_result == AV_STATUS_INFECTED:
        copy_s3_object(s3_object)
